// -*- C++ -*-
//
// Package:    StandardCandle
// Class:      StandardCandle
// 
/**\class StandardCandle StandardCandle.cc bbockelm/StandardCandle/src/StandardCandle.cc
...
class StandardCandle : public edm::OutputModule {
   public:
      explicit StandardCandle(const edm::ParameterSet&);
      ~StandardCandle();
...
//
StandardCandle::StandardCandle(const edm::ParameterSet& pset) :
   OutputModule(pset),
   ...
   m_triggerCount(0)
{
   ...
   if (selectionStrategy == "smallestFirst") {
      ...
      ex << "Invalid StandardCandle selection strategy: " << selectionStrategy;
      throw ex;
   }
...
void
StandardCandle::write(edm::EventPrincipal const& e)
{
   using namespace edm;
   ...
   edm::LogInfo("StandardCandle") << "StandardCandle read out " << ctr << " products.";
   
}

// ------------ method called when starting to processes a run  ------------
void 
StandardCandle::respondToOpenInputFile(edm::FileBlock const& fb)
{
   TTree *eventsTree = fb.tree();
   if (!eventsTree)
   {
      edm::Exception ex(edm::errors::ProductNotFound);
      ex << "StandardCandle was run with a TFile missing an events TTree.";
      throw ex;
   }
...
void
StandardCandle::computeProducts(edm::EventPrincipal const& e)
{
   using namespace edm;
   ...
   {
      edm::Exception ex(edm::errors::ProductNotFound);
      ex << "StandardCandle invoked computeProducts without an events TTree.";
      throw ex;
   }
   ...
   for (Provenances::iterator itProv = provenances.begin(), itProvEnd = provenances.end();
      ...
      if (!branch)
      {
         LogWarning("StandardCandle") << "Ignoring missing branch " << branchName;
         continue;
      }
   ...
   LogDebug("StandardCandle") << "Threshold is " << threshold << " of " << totalSize << " bytes.";
   ...
   LogInfo("StandardCandle") << "Reading " << m_products.size() << " of " << m_all_products.size() << " products.  Aggregate branch size is " << m_currentUsage << " of " << totalSize << " bytes.";

}

void
StandardCandle::fillSmallestFirst(ProductInfos const& all_products, Long64_t threshold)
{
   for (ProductInfos::const_iterator it = all_products.begin(), itEnd = all_products.end();
       ...
       LogDebug("StandardCandle") << "Adding label " << it->tag().label() << ", size " << it->size() << "; current usage is " << m_currentUsage << " of " << threshold << " bytes.";
   }
}

void
StandardCandle::fillLargestFirst(ProductInfos const& all_products, Long64_t threshold)
{
   m_currentUsage = 0;
...
void
StandardCandle::fillRoundRobin(ProductInfos const& all_products, Long64_t threshold)
{
   ...
   while (m_currentUsage < threshold)
   {
      if (useSmallest) {
         ...
         currentSmallest++;
         useSmallest = false;
         LogDebug("StandardCandle") << "Adding label " << pi.tag().label() << ", size " << pi.size() << "; current usage is " << m_currentUsage << " of " << threshold << " bytes.";
      } else
      {
...
void
StandardCandle::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
   ...
   desc.addUntracked<unsigned int>("percentBranches", 100)
     ->setComment("Control the percent of branches StandardCandle will read out.\n"
                  ...
                  "been read; thus, StandardCandle will likely read out more than this amount.");
   ...
   desc.addUntracked<unsigned int>("triggerFactor", 0)
      ->setComment("Controls the trigger rate.  Once every 'triggerFactor' events, StandardCandle\n"
   ...
   descriptions.add("StandardCandle", desc);
}

//define this as a plug-in
DEFINE_FWK_MODULE(StandardCandle);
